...
class DnCNN(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=64, nb=17, act_mode='BR', device="cuda:2"):
        """
        # ------------------------------------
    ...
    def get_noise(self, x):
        with torch.no_grad():
            img = torch.from_numpy(np.float32(x)).permute(2, 0, 1)[None, ...]
            res = self.model(img.to(self.device))[
